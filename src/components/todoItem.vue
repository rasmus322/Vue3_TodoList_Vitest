...
import type { ITask } from '@/stores/todoList';

const props = defineProps<{ task: ITask; deleteTask: (task: ITask) => void; changeTaskStatus: (task: ITask) => void  }>()

</script>

<template>
  <li class="todo_item"  :class="{done: props.task.done === true}">
    <button class="btn done_btn" @click="props.changeTaskStatus(props.task)">
      <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 48 48">
        <path fill="#43A047" d="M40.6 12.1L17 35.7 7.4 26.1 4.6 29 17 41.3 43.4 14.9z"></path>
      </svg>
    </button>
    {{ props.task.name }}
    <button class="btn delete_btn" @click="props.deleteTask(props.task)">
